...
class SyncService {
  ...
  Future<Map<String, dynamic>> syncPendingProducts(String token) async {
    ...
    try {
      ...
      print('🔄 Starting sync for ${pendingProducts.length} pending products...');
        ...
        try {
          ...
          if (result['success']) {
            ...
            if (result['authentication_error'] == true) {
              ...
              print('⚠️ Marked ${product['name']} as failed (validation error)');
            }
          }
        } catch (e) {
          failedCount++;
  ...
  Future<Map<String, dynamic>> _syncSingleProduct(Map<String, dynamic> product, String token) async {
    try {
      ...
      final bool hasLocalImage = localImagePath != null && File(localImagePath).existsSync();
      ...
      if (hasLocalImage) {
        try {
          ...
          var request = http.MultipartRequest('POST', Uri.parse('${ApiService.baseUrl}/api/officer/products'));
          ...
          request.files.add(await http.MultipartFile.fromPath('image', localImagePath));

          var streamedResponse = await request.send();
          mpResponse = await http.Response.fromStream(streamedResponse);
          
          if (mpResponse.statusCode != 201) {
            print('⚠️ Image upload failed (${mpResponse.statusCode}). Falling back to data-only sync.');
            mpResponse = null; // Trigger fallback
          }
        } catch (e) {
          print('⚠️ Image upload exception: $e. Falling back to data-only sync.');
          mpResponse = null;
        }
      }
      ...
      if (mpResponse != null) {
        ...
        if (!productData.containsKey('image_url')) {
          productData['image_url'] = product['image_url'] ?? 'https://via.placeholder.com/150';
        }
      ...
      if (response.statusCode >= 400) {
        final responseData = jsonDecode(response.body);
        final errorMsg = responseData['message'] ?? 'HTTP Error ${response.statusCode}';
  ...
  Future<void> _handleProductStockAndItems(
    ...
    try {
      final initialStock = int.tryParse(localProduct['initial_stock']?.toString() ?? '0') ?? 0;
          ...
          for (int i = 1; i <= initialStock; i++) {
            try {
              final itemResponse = await apiService.createProductItem({
                'product_id': serverProduct['id'],
                'qr_code': '${serverProduct['sku']}-${i.toString().padLeft(4, '0')}',
                'status': 'available',
              }, token);
              ...
              if (itemResponse['success'] == true) {
                ...
                print('⚠️ Failed to create item $i: ${itemResponse['message']}');
              }
            } catch (e) {
  ...
  Map<String, dynamic> _prepareProductForApi(Map<String, dynamic> product) {
    ...
    final productData = {
      ...
      'unit': unit,
      'buying_price': double.tryParse(product['base_price']?.toString() ?? '0') ?? 0.0,
      'selling_price': double.tryParse(product['selling_price']?.toString() ?? '0') ?? 0.0,
      'category_name': product['category_name'] ?? 'General',
      'duka_id': dukaId,
      // Remove initial_stock from product creation, will add stock separately
      'track_items': product['track_items'] == 1 || product['track_items'] == true,
  ...
  Future<Map<String, int>> getSyncStatusSummary() async {
    ...
    return {
      ...
      'total': (pendingResult.first['count'] as int) + (syncedResult.first['count'] as int),
    };
  }

  /// Force sync a specific product by local_id
  Future<Map<String, dynamic>> syncSpecificProduct(int localProductId, String token) async {
    final db = await _dbService.database;
  ...
  Future<Map<String, dynamic>> syncAllPendingData(String token) async {
    ...
    final stockResult = results['stock_movements'] as Map<String, dynamic>? ?? {};
    ...
    final overallSuccess = allProductsSuccess && allItemsSuccess && allStockSuccess;
    
    final totalSynced = (productsResult['synced_count'] as int? ?? 0) + 
                       (itemsResult['synced_count'] as int? ?? 0) + 
                       (stockResult['synced_count'] as int? ?? 0);
    
    final totalFailed = (productsResult['failed_count'] as int? ?? 0) + 
  ...
  Future<bool> _syncSingleProductItem(Map<String, dynamic> item, String token) async {
    try {
      print('📦 Syncing product item: ${item['qr_code']}');
      
      final db = await _dbService.database;
      
      // 1. Resolve Product Server ID
  ...
  Future<void> syncProductItemsForProduct(int productServerId, int productLocalId) async {
    ...
    try {
        // 1. Pull from server
        final apiService = ApiService();
        final response = await apiService.getProductItemsByProductId(productServerId, token);

        if (response['success'] == true && response['data'] != null) {
            final items = response['data'] as List;
            if (items.isNotEmpty) {
                await _dbService.saveSyncedProductItems(items, productLocalId);
                print('✅ Pulled and saved ${items.length} items for product $productLocalId');
            }
        }
        ...
        if (pendingItems.isNotEmpty) {
            print('🔄 Pushing ${pendingItems.length} pending items for product $productLocalId');
            for (var item in pendingItems) {
                await _syncSingleProductItem(item, token);
            }
        }
    } catch (e) {
        print('💥 Error during product items sync for product $productLocalId: $e');
    }
  }

  /// Force sync a specific product item by local_id
  Future<Map<String, dynamic>> syncSpecificProductItem(int localItemId, String token) async {
    final db = await _dbService.database;
    ...
    if (items.isEmpty) {
      return {
        'success': false,
        'message': 'Item not found or already synced',
      };
    }
  ...
  Future<bool> _syncSingleStockMovement(Map<String, dynamic> movement, String token) async {
    try {
      ...
      if (productServerId == null) {
        print('⏳ Parent product not synced yet for movement ${movement['local_id']}');
        return false;
      }

      final int quantity = movement['quantity'] as int;
      
      // The API only supports adding stock (positive quantity)
      if (quantity > 0) {
        ...
        if (response['success'] == true) {
         ...
         print('⚠️ Negative stock movement ($quantity) skipped. API requires positive quantity.');
      }
  ...
  Future<String?> getAuthToken() async {
    try {
      ...
      } else if (userData['data'] != null && userData['data']['access_token'] != null) {
    ...
    try {
      ...
      if (hasToken) {
        try {
          ...
          print('🌐 API connectivity: ${testResponse.statusCode == 200 ? "OK" : "Issue (${testResponse.statusCode})"}');
        } catch (e) {
  ...
  Future<Map<String, dynamic>> getDetailedSyncInfo() async {
    ...
    try {
      ...
      final pendingProducts = allProducts.where((p) => p['sync_status'] == statusPending).toList();
      final syncedProducts = allProducts.where((p) => p['sync_status'] == statusSynced).toList();
      ...
      return {
        ...
        'pending_products_sample': pendingProducts.take(3).map((p) => {
          'name': p['name'],
          'local_id': p['local_id'],
  ...
  Future<Map<String, dynamic>> syncSpecificSale(int localSaleId, String token) async {
    final db = await _dbService.database;
    
    try {
      // 1. Get Sale
      final sales = await db.query('sales', where: 'local_id = ?', whereArgs: [localSaleId]);
      if (sales.isEmpty) return {'success': false, 'message': 'Sale not found'};
      final sale = sales.first;

      // 2. Get Items
      final items = await db.query('sale_items', where: 'sale_local_id = ?', whereArgs: [localSaleId]);
      
      // 3. Prepare Items Data
      List<Map<String, dynamic>> apiItems = [];
      for (var item in items) {
         // Resolve Product Server ID
         final prodRes = await db.query('products', columns: ['server_id'], where: 'local_id = ?', whereArgs: [item['product_local_id']]);
         int? productServerId;
         if (prodRes.isNotEmpty) productServerId = prodRes.first['server_id'] as int?;
         
         if (productServerId == null) {
           return {'success': false, 'message': 'Cannot sync sale: Contains unsynced products'};
         }
      ...
      if (response['success'] == true) {
         final serverId = response['data']['sale']['id'];
         await _dbService.updateSaleServerId(localSaleId, serverId);
         return {'success': true, 'server_id': serverId};
      } else {
         return {'success': false, 'message': response['message'] ?? 'Unknown API error'};
      }
    } catch (e) {
  ...
  Future<void> syncAllPendingSales(String token) async {
    final db = await _dbService.database;
    final pendingSales = await db.query('sales', where: 'sync_status = ?', whereArgs: [statusPending]);
  ...
  Future<bool> _syncSingleStock(Map<String, dynamic> stock, String token) async {
    try {
      ...
      if (response['success'] == true) {
        ...
        print('⚠️ Failed to sync stock: ${response['message'] ?? 'Unknown error'}');
        return false;
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> syncAllPendingDataIncludingStocks(String token) async {
    ...
    final productsResult = results['products'] as Map<String, dynamic>? ?? {};
    final itemsResult = results['product_items'] as Map<String, dynamic>? ?? {};
    final stockMovementsResult = results['stock_movements'] as Map<String, dynamic>? ?? {};
    final stocksResult = results['stocks'] as Map<String, dynamic>? ?? {};
    
    final allProductsSuccess = productsResult['success'] as bool? ?? false;
    final allItemsSuccess = itemsResult['success'] as bool? ?? false;
    final allStockMovementsSuccess = stockMovementsResult['success'] as bool? ?? false;
    final allStocksSuccess = stocksResult['success'] as bool? ?? false;
    
    final overallSuccess = allProductsSuccess && allItemsSuccess && allStockMovementsSuccess && allStocksSuccess;
