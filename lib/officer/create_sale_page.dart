...
class CreateSalePage extends StatefulWidget {
  ...
  const CreateSalePage({super.key, this.proformaInvoice, this.onSaleCreated});
...
class _CreateSalePageState extends State<CreateSalePage> {
  ...
  void _loadProformaData() {
    try {
      ...
      if (invoice['items_data'] != null) {
        final rawItems = (jsonDecode(invoice['items_data']) as List).cast<Map<String, dynamic>>();
        // Map product_local_id back to local_id for compatibility with sale logic
        final items = rawItems.map((item) {
          if (!item.containsKey('local_id') && item.containsKey('product_local_id')) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Proforma invoice loaded'), backgroundColor: Colors.green),
          );
        }
      });
    } catch (e) {
  ...
  Future<void> _addProductToCartByCode(String code) async {
    ...
    try {
      ...
      if (product != null) {
        ...
        if (product['scanned_item_id'] != null && product['scanned_item_status'] != 'available') {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Item not available. Status: ${product['scanned_item_status']}.'),
  ...
  void _addToCart(Map<String, dynamic> product) {
    setState(() {
      ...
      if (product['scanned_item_id'] != null) {
        ...
        final existingProductIndex = _cart.indexWhere((item) => 
  ...
  Future<void> _saveCartAsDraft() async {
    if (_cart.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Cannot save empty cart as draft'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    final shouldSave = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('Save Draft', style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Enter a note to identify this draft (optional):', style: GoogleFonts.plusJakartaSans(color: Colors.white70)),
    ...
    if (shouldSave == true) {
      ...
      final draft = {
        'customer_data': _selectedCustomer != null ? jsonEncode(_selectedCustomer) : null,
  ...
  Future<void> _showDraftsDialog() async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF0A1B32),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text('Saved Drafts', style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.bold)),
        content: drafts.isEmpty
            ? Padding(
                padding: const EdgeInsets.all(20),
                child: Text('No saved drafts found.', style: GoogleFonts.plusJakartaSans(color: Colors.white54)),
              )
            : SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final items = (jsonDecode(draft['items_data']) as List).length;
        ...
        actions: [
          TextButton(
            ...
            child: Text('Close', style: GoogleFonts.plusJakartaSans(color: Colors.white54)),
          ),
        ],
      ),
    );
  ...
  Future<void> _loadDraft(Map<String, dynamic> draft) async {
    if (_cart.isNotEmpty) {
      final confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: const Color(0xFF0A1B32),
          title: Text('Overwrite Cart?', style: GoogleFonts.plusJakartaSans(color: Colors.white)),
          content: Text('Loading this draft will replace your current cart items.', style: GoogleFonts.plusJakartaSans(color: Colors.white70)),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Overwrite', style: TextStyle(color: Colors.redAccent))),
          ],
        ),
      );
    ...
    setState(() {
      _cart.clear();
      final items = (jsonDecode(draft['items_data']) as List).cast<Map<String, dynamic>>();
      _cart.addAll(items);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Draft loaded successfully'), backgroundColor: Colors.green),
      );
    }
  }
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          backgroundColor: const Color(0xFF0A1B32),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          title: Text('Confirm Checkout', style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
          content: SingleChildScrollView(
            ...
            children: [
              ...
              Container(
                decoration: BoxDecoration(
                  color: _isLoan ? Colors.orange.withOpacity(0.1) : Colors.white.withOpacity(0.05),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _isLoan ? Colors.orange.withOpacity(0.5) : Colors.transparent),
                ),
                child: CheckboxListTile(
                  title: Text('Mark as Loan', style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 14)),
                  subtitle: _isLoan ? Text('Payment pending', style: GoogleFonts.plusJakartaSans(color: Colors.orange, fontSize: 11)) : null,
                  value: _isLoan,
                  onChanged: (val) {
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.pop(context);
                  _processCheckout(discount, note);
              },
              ...
              child: Text('Confirm Sale', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, fontSize: 13)),
            ),
          ],
        );
  ...
  Future<void> _processCheckout(double discount, String note) async {
    setState(() => _isLoading = true);
    try {
      final db = await DatabaseService().database;
      ...
      final tenantId = officerRes.isNotEmpty ? officerRes.first['tenant_id'] : null;
      final dukaId = dukaRes.isNotEmpty ? dukaRes.first['server_id'] : null;
      final customerId = _selectedCustomer?['server_id'] ?? _selectedCustomer?['local_id'];
      ...
      final saleData = {
        ...
        'profit_loss': 0.0,
        'is_loan': _isLoan ? 1 : 0,
        'due_date': _isLoan ? DateTime.now().add(const Duration(days: 30)).toIso8601String() : null,
      ...
      if (widget.proformaInvoice != null && widget.proformaInvoice!['id'] != null) {
        await DatabaseService().deleteProformaInvoice(widget.proformaInvoice!['id']);
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isSynced ? 'Sale completed and synced!' : 'Sale saved locally. Will sync when online.'), 
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              PopupMenuItem(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.save_as_outlined, color: Colors.white70, size: 20),
                    const SizedBox(width: 12),
                    Text('Save Draft', style: GoogleFonts.plusJakartaSans(color: Colors.white)),
                  ],
                ),
              ),
              PopupMenuItem(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.restore_page_outlined, color: Colors.white70, size: 20),
                    const SizedBox(width: 12),
                    Text('Load Drafts', style: GoogleFonts.plusJakartaSans(color: Colors.white)),
                  ],
                ),
              ),
            ],
