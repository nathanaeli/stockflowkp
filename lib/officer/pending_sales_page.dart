...
class _PendingSalesPageState extends State<PendingSalesPage> {
  ...
  Future<void> _loadPendingSales() async {
    ...
    try {
      ...
      for (var sale in sales) {
        String customerName =
            AppLocalizations.of(context)?.walkInCustomer ?? 'Walk-in';
  ...
  Future<void> _syncSale(int index) async {
    ...
    try {
      ...
      if (token == null) {
        throw Exception(
          AppLocalizations.of(context)?.notAuthenticated ?? 'Not authenticated',
        );
      }

      final result = await syncService.syncSpecificSale(
        sale['local_id'],
        token,
      );
      ...
      if (mounted) {
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppLocalizations.of(context)?.saleSyncedSuccessfully ??
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppLocalizations.of(
                      context,
                    )?.syncFailedWithMessage(result['message']) ??
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              AppLocalizations.of(
                    context,
                  )?.syncedCountOfTotal(successCount, _pendingSales.length) ??
                  'Synced $successCount of ${_pendingSales.length} sales',
            ),
            backgroundColor:
                successCount == _pendingSales.length
                    ? Colors.green
                    : Colors.orange,
          ),
        );
                  ...
                  ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          AppLocalizations.of(context)?.allSalesSynced ??
