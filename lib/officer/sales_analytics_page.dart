...
class _SalesAnalyticsPageState extends State<SalesAnalyticsPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final categoryData = await DatabaseService().getSalesByCategory(
        _selectedDateRange,
      );
      final topProductsData = await DatabaseService().getTopSellingProducts(
        _selectedDateRange,
      ...
      for (int i = 0; i <= daysDifference; i++) {
        ...
        if (_isComparing && _compareDateRange != null) {
          ...
          fullCompareData.add({
            ...
            'total':
                (existingCompareEntry['total_amount'] as num?)?.toDouble() ??
                0.0,
  ...
  Future<void> _exportExcel() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${localizations.errorExportingExcel}: $e'),
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text(
                localizations.last7DaysPerformance,
              ...
              pw.Table.fromTextArray(
                ...
                headerDecoration: const pw.BoxDecoration(
                  color: PdfColor.fromInt(0xFF1E4976),
                ),
  ...
  Widget build(BuildContext context) {
    final double totalRevenue = _salesData.fold(
      0,
      (sum, item) => sum + (item['total'] as double),
    );
    final int totalSales = _salesData.fold(
      0,
    ...
    if (_isComparing) {
      compareTotalRevenue = _compareSalesData.fold(
        0,
        (sum, item) => sum + (item['total'] as double),
      );
    }
    ...
    double maxVal = _salesData.fold(
      0.0,
      (max, item) =>
          (item['total'] as double) > max ? (item['total'] as double) : max,
    );
    if (_isComparing) {
      final double compareMax = _compareSalesData.fold(
        0.0,
        (max, item) =>
            (item['total'] as double) > max ? (item['total'] as double) : max,
      );
  ...
  Widget _buildCategoryPieChart() {
    if (_categorySales.isEmpty) {
      return Container(
        ...
        child: Center(
          child: Text(
            AppLocalizations.of(context)!.noCategoryDataAvailable,
    ...
    final double totalRevenue = _categorySales.fold(
      0,
        ...
        children: [
            ...
            child: PieChart(
              PieChartData(
                    ...
                    setState(() {
                      ...
                      if (event is FlTapUpEvent) {
                        ...
                        if (index >= 0 && index < _categorySales.length) {
                          ...
                          // Toggle selection
                          _selectedCategoryId =
                              (_selectedCategoryId == catId) ? null : catId;
                          _loadTopProducts();
                        }
                      }
                    });
                ...
                sections: List.generate(_categorySales.length, (i) {
                  ...
                  final percentage =
                      totalRevenue > 0
                          ? (value / totalRevenue * 100).toStringAsFixed(1)
                          : '0.0';
          ...
          Column(
            children: List.generate(_categorySales.length, (i) {
              final cat = _categorySales[i];
              final name =
                  cat['name'] ?? AppLocalizations.of(context)!.uncategorized;
              final value = (cat['total'] as num).toDouble();
              final color = colors[i % colors.length];
              final isSelected = _selectedCategoryId == cat['category_id'];
